...
impl<T, F, OldF, Tail: Clone, S, FT> InsertOrReplace<T, F, Here, S, FT>
    for HCons<T, OldF, Tail, S, FT>
{
...
impl<T, F, Head, HeadF: Clone, Tail, Idx, S, FT> InsertOrReplace<T, F, There<Idx>, S, FT>
    for HCons<Head, HeadF, Tail, S, FT>
where
...
pub trait Execute {
    ...
    async fn execute(
        ...
        state_id: u8,
        bytes: Vec<u8>,
        fs: &mut RemoteFileSystem<Self::State, Self::FileType>,
...
impl<T: Decodable, F, Tail, S: StreamReceiver, FT> Execute for HCons<T, F, Tail, S, FT>
where
    F: for<'a> Fn(
        u8,
        T::Output,
        &'a mut RemoteFileSystem<S, FT>,
    ) -> Pin<Box<dyn Future<Output = Result<(), FileHandleStatus>> + Send + 'a>>,
...
impl<H: Execute<State = S, FileType = FT>, S: StreamReceiver, FT> ChainBuilder<'_, H, S, FT> {
    pub fn chain<T: Decodable, F, Idx>(&mut self, f: F) -> ChainBuilder<H::Output, S, FT>
    where
